...
const localizationValidator = (instance) => {
  ...
  if (localization.default.length != 2) {
    ...
    errors.push(
      `Localization error: Default locale should be two-letter language code, got: ${localization.default}`
    );
  }
  ...
  if (notTwoLetterCode.length > 0) {
    notTwoLetterCode.map((locale) => {
      errors.push(
        `Localization error: Locale should be two-letter language code, got: ${locale}`
      );
    });
  }
  ...
  if (localization.locales.includes(localization.default)) {
    ...
    errors.push(
      `Localization error: Default locale should not appear in "locales"`
    );
  }
  ...
  if (!localization.uri.includes("/{locale}.json")) {
    errors.push(
      `Localization error: URI should be of format "<protocol>://<hash>/{locale}.json"`
    );
  }
